  ...
  <view :style="`${rootStyle};display: inline-block;`">
    <view :class="`wd-sticky ${customClass}`" :style="stickyStyle" :id="styckyId">
      <view class="wd-sticky__container" :style="containerStyle">
        <wd-resize @resize="handleResize" custom-style="display: inline-block; box-sizing: content-box;">
          <slot />
        </wd-resize>
      </view>
    </view>
...
const rootStyle = computed(() => {
  const style: CSSProperties = {
    ...
    height: addUnit(stickyState.height + 'px'),
    width: addUnit(stickyState.width + 'px')
  }
...
const containerStyle = computed(() => {
  const style: CSSProperties = {
    ...
    top: addUnit(stickyState.top + 'px')
  }
  return objToStyle(style)
})
